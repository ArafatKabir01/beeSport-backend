...
const AdministratorSettings = require("./model");
...
const getAdministratorSettings = async (req, res, next) => {
    try {
      const generalSettingSchema = await AdministratorSettings.findOne();
...
const updateAdministratorSettings = async (req, res, next) => {
    upload.fields([
      {
        name: "site_logo",
        maxCount: 1
      ...
      if (generalSettingSchema) {
        ...
        const administrationSettings = new AdministratorSettings({
          company_name,
          site_title,
